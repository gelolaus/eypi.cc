<template>
  <section class="relative flex min-h-screen w-full flex-col items-center justify-center px-4 py-28">
    <div class="w-full max-w-md">
      ...
      <!-- Loading event -->
      <div v-if="loadingEvent" class="space-y-3 text-center">
        <div class="mx-auto h-8 w-48 animate-pulse rounded-lg bg-gray-200 dark:bg-slate-800/60" />
        <div class="mx-auto h-4 w-32 animate-pulse rounded-lg bg-gray-200 dark:bg-slate-800/60" />
      </div>
      ...
      <div v-else-if="!event" class="text-center">
        ...
        <router-link to="/" class="mt-6 inline-block font-mono text-xs uppercase tracking-widest text-[#DEAC4B] hover:underline">← Back to home</router-link>
      </div>
...
interface Ticket { firstName: string; lastName: string; email: string; qrToken: string }
...
onMounted(async () => {
  try {
    ...
    const data = await res.json() as { status: string; message?: string; event?: EventInfo & { isLead: boolean } }
...
async function lookup() {
  ...
  try {
    ...
    if (!res.ok) throw new Error(data.message ?? 'No ticket found.')
    ticket.value = data.ticket!

    await nextTick()
    await renderQR(data.ticket!.qrToken)
  } catch (err: unknown) {
    lookupError.value = err instanceof Error ? err.message : 'No ticket found.'
  } finally {
