<template>
  ...
  >
    ...
    >
      ...
      <h2 class="mb-4 text-center font-mono text-xl font-bold text-[#34418F]">
        VERIFY YOUR ACCOUNT
      </h2>

      <p
        v-if="error"
        class="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 font-mono text-sm text-red-700"
      >
        {{ error }}
      </p>
      ...
      <p
        v-else-if="!token"
        class="mb-6 font-mono text-sm text-gray-600"
      >
        No verification token found. Please use the link from your email.
      </p>

      <template v-else>
        <p class="mb-8 font-mono text-sm text-gray-600">
        ...
        <button
          ...
          :class="[
            'w-full rounded-lg bg-[#DEAC4B] px-4 py-4 font-mono text-base font-bold uppercase tracking-wider text-white transition-all duration-200',
