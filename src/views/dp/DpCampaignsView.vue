<template>
  <section class="relative mx-auto w-full max-w-5xl px-6 py-16">
    ...
    <div v-if="loading" class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <div v-for="i in 3" :key="i" class="h-40 animate-pulse rounded-3xl bg-gray-200 dark:bg-slate-800/60" />
    </div>
    ...
    >
      ...
      <p class="mt-3 font-mono text-xs leading-relaxed text-gray-400 dark:text-slate-500">
        Upload a transparent PNG frame to generate a shareable DP Blast link.
      </p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        ...
        <div class="mt-1 flex items-center gap-2 font-mono text-xs text-gray-500 dark:text-slate-400">
          <span class="text-2xl font-black text-[#34418F] dark:text-slate-200">{{ c.downloadCount }}</span>
          <span class="uppercase tracking-widest">download{{ c.downloadCount === 1 ? '' : 's' }}</span>
        </div>

        <p class="truncate font-mono text-[0.7rem] text-gray-400 dark:text-slate-500">eypi.cc/dp/{{ c.id }}</p>
...
async function copyLink(id: string) {
  try {
    await navigator.clipboard.writeText(`https://eypi.cc/dp/${id}`)
...
async function remove(id: string, title: string) {
  if (!confirm(`Delete "${title}"?\n\nThis permanently removes the campaign and its public page. This cannot be undone.`)) return
  deletingId.value = id
  try {
...
onMounted(async () => {
  try {
    ...
    const data = await res.json() as { status: string; message?: string; campaigns?: Campaign[] }
